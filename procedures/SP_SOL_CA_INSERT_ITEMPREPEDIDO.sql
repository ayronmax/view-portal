CREATE OR REPLACE PROCEDURE DBCONTROL8201001.SP_SOL_CA_INSERT_ITEMPREPEDIDO (IN COD_NEMP SMALLINT,
IN COD_PRE_PEDIDO BIGINT,
IN SEQ_ITEM SMALLINT,
IN COD_OCORRENCIA SMALLINT,
IN COD_RED VARCHAR(20),
...
BEGIN 
   ...
   declare codbar VARCHAR(20);
   
   set CodReduzido = select VDPRDPRD_CODR FROM CADPRD01 where VDPRDPRD_CODCMP = COD_RED AND
                     VDPRDPRD_FLAG = 'A';
   ...
   set codprd = select cast(concat(repeat('0',3 - length(cast(VDPRDPRD_CFAM as varchar(3)))),cast(VDPRDPRD_CFAM as varchar(3))) ||
    ...
    IF ITEM <> NULL THEN
    SET STATUSMSG = 1;
SET MSG = 'ITEM JA EXISTE NESSA SEQUENCIA';
ELSE   
...
SET MSG = 'ITEM INCLUIDO COM SUCESSO';
 ELSE
SET codbar = LEFT(COD_RED, LOCATE('C', COD_RED, 1) - 1);

IF codbar is null then
   SET codbar = COD_RED;
END IF;   

SET codprd = SELECT VDPRDBAR_CODPRD FROM VDPRBA01 WHERE VDPRDBAR_UNIDADE = 2 AND VDPRDBAR_CODBAR = codbar;

set CodReduzido = select VDPRDPRD_CODR FROM CADPRD01 where VDPRDPRD_FLAG = 'A' AND 
cast(concat(repeat('0',3 - length(cast(VDPRDPRD_CFAM as varchar(3)))),cast(VDPRDPRD_CFAM as varchar(3))) || 
concat(repeat('0',3 - length(cast(VDPRDPRD_NRO as varchar(3)))), cast(VDPRDPRD_NRO as varchar(3))) as int) = codprd;
