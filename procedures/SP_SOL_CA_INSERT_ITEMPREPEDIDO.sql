CREATE OR REPLACE PROCEDURE SP_SOL_CA_INSERT_ITEMPREPEDIDO (IN COD_NEMP SMALLINT,
...
LANGUAGE SQL  
BEGIN 
   #ORIGEM = (L = PORTAL / R = SFA VIA PORTAL / T = CA)
   ...
   set CodReduzido = select VDPRDPRD_CODR FROM CADPRD01 where VDPRDPRD_CODCMP = COD_RED AND VDPRDPRD_FLAG = 'A';
      ...
      IF COD_NEMP <> NULL AND COD_PRE_PEDIDO <> NULL THEN
         ...
         IF prepedido <> NULL THEN
            IF  COD_NEMP = null or
                ...
                VALOR_LIQ = NULL OR 
                VALOR_BRUTO = NULL OR
                (valorLiquidoUmaCaixa = null or valorLiquidoUmAvulso = null) THEN    
                SET STATUSMSG = 1;
            ...
            ELSE
                ...
                IF ORIGEM = 'L' and ORIGEM = null THEN 
                   INSERT INTO VDPEDIPP(VDPEDIPP_NREMP      ,      
                                        VDPEDIPP_PRE_PED    ,     
                                        VDPEDIPP_ITEM       ,                    
                                        VDPEDIPP_OCOKD      ,         
                                        VDPEDIPP_CODR       ,
