		  ...
		  END IF;
			...
			if prepedido <> null then 
			   ...
			   if NEMP             = null or
			       ...
			       SET CODCLI = SELECT cast(concat(concat(repeat('0',4-length(cast(vdclicli_regi as varchar(4)))),cast(vdclicli_regi as varchar(4))),concat(repeat('0',4-length(cast(vdclicli_num as varchar(4)))),cast(vdclicli_num as varchar(4)))) as int) FROM CADCLI06 WHERE VDCLICLI_CGC = CAST(CGCNOVO AS DECIMAL(15,0));
			       SET codven = SELECT VDCLICLI_VEN FROM CADCLI06 WHERE VDCLICLI_CGC = CAST(CGCNOVO AS DECIMAL(15,0));
			       SET intTipoCobra = SELECT VDCADTCC_COD FROM VDCADTCC WHERE VDCADTCC_COD_CIA = tipocob;
			       SET intCondPag = SELECT VDCADPAG_COD FROM CONDPG06 WHERE VDCADPAG_CODCIA = condpagamento;
