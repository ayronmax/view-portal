...
SELECT 
  ...
  cast(concat(concat(repeat('0',4-length(cast(vdclicli_regi as varchar(4)))),cast(vdclicli_regi as varchar(4))),concat(repeat('0',4-length(cast(vdclicli_num as varchar(4)))),cast(vdclicli_num as varchar(4)))) as INT) CODIGO_CLIENTE_CHAVE,
...
CREATE OR REPLACE VIEW VW_GRUPO_ANALISE
AS
SELECT
VDCLIROC_ROMA CODIGO_GRUPO_ANALISE,
VDCLIROC_NOMEGRP NOME_GRUPO_ANALISE,
CODIGO_CLIENTE
FROM ROCLI01
LEFT JOIN VW_DADOS_CLIENTE ON CODIGO_CLIENTE_CHAVE = VDCLIROC_CCLI
WHERE VDCLIROC_CCLI > 0 AND
      CODIGO_CLIENTE_CHAVE IS NOT NULL;
