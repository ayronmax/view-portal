...
SELECT ATIVO,
       ...
       TO_DATE(SUBSTRING(cast(DATA_VENCIMENTO as varchar(8)),7,2) || SUBSTRING(cast(DATA_VENCIMENTO as varchar(8)),5,2) || SUBSTRING(cast(DATA_VENCIMENTO as varchar(8)),1,4), 'DDMMYYYY') AS DATA_VENCIMENTO,
       DESCONTO_FINANCEIRO,
       NUMERO_PEDIDO,
       PERCENTUAL_DESCONTO,
       STATUS_PEDIDO,
       ...
       NFISCAL_INI,
       NFISCAL_ULT
FROM VW_DADOS_HISTORICO_PEDIDO_CAPA
LEFT JOIN VW_DADOS_VDPEDFLC ON NUMERO_PEDIDO_VDPEDFLC = NUMERO_PEDIDO
LEFT JOIN VW_DADOS_CLIENTE ON REGIAO_CLIENTE = REG_CLI_PED
AND NUMERO_CLIENTE = NUM_CLI_PED
LEFT JOIN CONDPG01 ON vdcadpag_cod = CODIGO_CONDICAO_PAGAMENTO_REC_ID
LEFT JOIN TPCOBR01 ON VDCADTCO_COD = CODIGO_TIPO_COBRANCA_REC_ID
